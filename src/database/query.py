...
class ReadOnly:
    ...
    def get_users(self):
        ...
        return [
            {'sky_username': user.sky_username, 'display_name': user.display_name, 'mod': user.mod, 'email': user.email}
            for user in queried]
    ...
    def get_courseids_from_thread(self, thread_id):
        queried = TagLine.query.filter(TagLine.thread_id == thread_id).join(Tag, TagLine.tag == Tag.id).add_column(
            Tag.course_id).all()
    ...
    def get_thread_by_order(self, order):
        if order is not None and order in {"RECENT", "LIKES", "POPULAR", "SEARCH"}:
            ...
            if order == "RECENT":
                queried = queried.order_by(Thread.timestamp.desc()).limit(10)
            elif order == "LIKES":
    ...
    def check_manual_thread_like(self, thread_id, user_id):
        return ThreadLikes.query.filter(ThreadLikes.thread_id == thread_id).filter(
            ThreadLikes.user_id == user_id).first()

    def check_thread_like(self, thread_id):
        if not current_user.is_authenticated: return None
        return self.check_manual_thread_like(thread_id, current_user.id)
    ...
    def jsonify_thread(self, thread):
        ...
        return {'thread_id': thread_id, 'title': title, 'likes': likes, 'display_name': display_name,
                'date': self.get_readable_day(date), \
    ...
    def get_top_comment(self, thread_id):
        ...
        return Comment.query.filter(Comment.thread_id == thread_id, Comment.main_comment,
                                    Comment.deleted == 0).order_by(Comment.likes.desc()).first()
    ...
    def check_comment_like(self, comment_id):
        if not current_user.is_authenticated: return None
        return CommentLikes.query.filter(CommentLikes.comment_id == comment_id).filter(
            CommentLikes.user_id == current_user.id).first()
    ...
    def get_comments_of_thread(self, thread_id):
        queried = Comment.query.filter(Comment.thread_id == thread_id).filter(Comment.main_comment).order_by(
            Comment.likes.desc()).all()
        return [{'sender': self.get_user_from_id(comment.user_id).display_name,
                 'timestamp': self.get_readable_day(comment.timestamp), 'body': comment.comment_body,
                 'is_liked': self.check_comment_like(comment.id) is not None, \
                 'likes': comment.likes, 'replies': self.get_all_replies(comment.id), 'comment_id': comment.id,
                 'reply': False, 'deleted': comment.deleted, 'edit_box': False} for comment in queried]

    def get_all_replies(self, parent_id):
        queried = Comment.query.join(CommentLine, CommentLine.child_comment_id == Comment.id).filter(
            CommentLine.parent_comment_id == parent_id).order_by(Comment.timestamp).all()
        return [{'sender': self.get_user_from_id(comment.user_id).display_name,
                 'timestamp': self.get_readable_day(comment.timestamp), 'body': comment.comment_body, \
                 'is_liked': self.check_comment_like(comment.id) is not None, 'likes': comment.likes,
                 'comment_id': comment.id, 'reply': False, 'deleted': comment.deleted, 'edit_box': False} for comment in
                queried]
