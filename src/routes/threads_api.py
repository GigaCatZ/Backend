...
def get_thread_info():
    ...
    return jsonify(status=True, thread_id=thread.id, author=read_queries.get_user_from_id(thread.user_id).display_name,
                   title=thread.question, is_liked=(read_queries.check_thread_like(thread.id) is not None), \
                   body=thread.body, timestamp=read_queries.get_readable_day(thread.timestamp), likes=thread.likes,
                   comments=read_queries.get_comments_of_thread(thread.id),
                   tags=read_queries.get_tags_from_thread(thread.id))
...
def like_thread():
    ...
    try:
        if not current_user.is_authenticated: return jsonify(status=False, liked_thread=False,
                                                             message="User is not logged in!", thread_id=thread_id,
                                                             new_likes=read_queries.get_thread_like_count(thread_id),
                                                             username=None)
        thread, liked, message = write_queries.upvote_thread(thread_id)
        return jsonify(status=True, liked_thread=liked, message=message, thread_id=thread.id, new_likes=thread.likes,
                       username=current_user.sky_username)
    except(AttributeError):
        jsonify(status=False, liked_thread=None, message="Thread does not exist", thread_id=None, new_likes=None,
                username=None)
    return jsonify(status=False, liked_thread=None, message="No thread ID sent", thread_id=None, new_likes=None,
                   username=None)
...
def edit_thread():
    ...
    if (user_id == thread.user_id):
        ...
        if new_question_tags == "":
            new_question_tags = set()
        else:
            new_question_tags = set(new_question_tags.split(','))
...
def info_for_edit_thread():
    ...
    return jsonify(courses=read_queries.display_all_tags(), selected_tags=read_queries.get_tags_from_thread(thread_id),
                   title=thread.question, body=thread.body)
...
def create_thread():
    ...
    if not current_user.is_authenticated:
        return jsonify(status=False, username=None, thread_id=None, title=None, tags=None,
                       message="Couldn't get the username")

    # This can probably be handled in frontend but yah
    if (question_title.strip() == ""):
        return jsonify(status=False, username=None, thread_id=None, title=None, tags=None,
                       message="Thread title required.")
    ...
    return jsonify(status=True, username=current_user.sky_username, thread_id=thread.id, thread_title=thread.question,
                   tags=tags, message="Thread has been created.")
