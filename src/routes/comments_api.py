...
def new_comment():
    if not current_user.is_authenticated:
        return jsonify(status=False, message="User is not logged in", thread_id=None, parent_id=None, comment_id=None,
                       username=None)

    comment_body = request.form.get("comment_body")

    if (comment_body is None or len(comment_body.strip()) == 0):
        return jsonify(status=False, message="Empty comment body", thread_id=None, parent_id=None, comment_id=None,
                       username=None)
    ...
    try:
        ...
        return jsonify(comment_id=comment_id, username=current_user.sky_username, thread_id=thread_id,
                       parent_id=parent_id, status=True, message="Comment created successfully")
    except(IntegrityError):
        return jsonify(status=False, message="Parent comment does not exist", thread_id=None, parent_id=None,
                       comment_id=None, username=None)
...
def like_comment():
    ...
    try:
        if not current_user.is_authenticated: return jsonify(status=False, liked_comment=False,
                                                             message="User is not logged in!", comment_id=comment_id,
                                                             new_likes=read_queries.get_comment_like_count(comment_id),
                                                             username=None)
        comment, liked, message = write_queries.upvote_comment(comment_id)
        return jsonify(status=True, liked_comment=liked, message=message, comment_id=comment.id,
                       new_likes=comment.likes, username=current_user.sky_username)
    except(AttributeError):
        jsonify(status=False, liked_comment=None, message="Comment does not exist", comment_id=None, new_likes=None,
                username=None)

    return jsonify(status=False, liked_comment=None, message="There's an error", comment_id=None, new_likes=None,
                   username=None)
