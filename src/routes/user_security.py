...
def authenticate():
    if (current_user.is_authenticated):
        return jsonify(is_logged_in=True, display_name=current_user.display_name,
                       sky_username=current_user.sky_username, mod=current_user.mod)
    else:
...
def register():
    ...
    if (username != "" and password != "" and display_name != "" and email != "" and \
            username is not None and password is not None and display_name is not None and email is not None):
        try:
            ...
            return jsonify(username=username, status=False,
                           message="Username, Display Name, or Email has already been taken")
...
def update_info():
    ...
    if password is None or not bcrypt.checkpw(password.encode('utf8'),
                                              read_queries.get_encrypted_password(username).encode('utf8')):
...
def forgot_password():
    ...
    return jsonify(status=True,
                   message='The request has been sent. Your new password has been sent to you via Email! If you don\'t see any email, please check your spam folder.')
